...
img = cv2.imread("Latihan/Gambar/4.jpeg", 0)
# Ulangi setiap piksel dan ubah nilai piksel menjadi biner menggunakan
# np.binary_repr() dan simpan dalam array
lst = []
for i in range(img.shape[0]):
    for j in range(img.shape[1]):
        lst.append(np.binary_repr(img[i][j], width=8))
        eight_bit_img = (
            np.array([int(i[0]) for i in lst], dtype=np.uint8) * 128
